...
| 설교 | 시리즈별 목록, 영상·음성 구분, 상세 화면에서 재생(유튜브·오디오 링크) |
| 기도 | 기도 타이머와 빠른 기록, 연속 일수·최근 7일 그래프, 기도제목 나눔(익명 가능)과 &lsquo;함께 기도&rsquo; |
| 소그룹 | 소그룹 목록과 소통방(채팅) |
| 더보기 | 로그인/로그아웃, 교회 정보(전화·지도·이메일 바로 연결), 헌금 계좌, 데이터 모드 |
| 관리자 | 공지·설교 등록/수정/삭제 (관리자 계정만) |
...
- `src/constants/theme.ts` — 색상 팔레트(라이트/다크 모드 모두 정의)
- `src/lib/data/sample-data.ts` — 샘플 모드에서 보이는 주보·공지·설교·기도제목·소그룹
- `app.json` — 앱 이름, 아이콘, 스플래시 색상
...
- 설교 영상 인앱 플레이어(현재는 링크를 브라우저로 엽니다)
