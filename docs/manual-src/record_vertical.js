...
const html = `<!doctype html><html lang="ko"><head><meta charset="utf-8"><style>
  * { margin:0; padding:0; box-sizing:border-box; }
  html,body { width:1080px; height:1920px; overflow:hidden; font-family:"WenQuanYi Zen Hei","Noto Sans CJK KR",sans-serif; }
  #stage { position:relative; width:1080px; height:1920px; background:linear-gradient(160deg,#f5efe3,#e6dbc7); }
  #header { position:absolute; top:0; left:0; right:0; height:132px; background:${NAVY}; display:flex; align-items:center; justify-content:center; z-index:5; }
  #header img { width:58px; height:58px; border-radius:14px; margin-right:16px; }
  #header .tt { color:#fff; font-weight:bold; font-size:34px; letter-spacing:.5px; }

  #content { position:absolute; top:132px; left:0; right:0; bottom:0; display:none; }
  #phoneWrap { position:absolute; left:50%; transform:translateX(-50%); top:44px; }
  #phone { width:404px; height:842px; border-radius:44px; background:#0d0d0d; padding:14px; box-shadow:0 22px 55px rgba(0,0,0,.30); }
  #screen { position:relative; width:100%; height:100%; border-radius:31px; overflow:hidden; background:#fff; }
  ...
  #cap { position:absolute; left:80px; right:80px; top:970px; }
  #cap .chip { display:inline-block; background:${NAVY}; color:#fff; font-weight:bold; font-size:26px; padding:9px 26px; border-radius:999px; margin-bottom:22px; }
  #cap h1 { color:#12303f; font-size:58px; line-height:1.2; margin-bottom:30px; }
  #cap ul { list-style:none; }
  #cap li { position:relative; color:#37505c; font-size:34px; line-height:1.42; padding-left:38px; margin:20px 0; }
  #cap li::before { content:''; position:absolute; left:2px; top:16px; width:14px; height:14px; border-radius:50%; background:#3a9fb0; }
  ...
  #title { position:absolute; inset:0; display:none; flex-direction:column; align-items:center; justify-content:center; text-align:center; padding:0 90px; z-index:10; background:linear-gradient(160deg,#f5efe3,#e6dbc7); }
  #title img.logo { width:190px; height:190px; border-radius:44px; margin-bottom:44px; box-shadow:0 20px 46px rgba(16,76,110,.30); }
  #title h1 { color:#12303f; font-size:74px; line-height:1.3; white-space:pre-line; }
  #title .sub { color:#6b7785; font-size:36px; margin-top:26px; }
  ...
  #barwrap { position:absolute; left:0; right:0; bottom:0; height:9px; background:rgba(16,76,110,.12); z-index:20; }
  #bar { height:100%; width:0; background:${NAVY}; }
</style></head><body>
<div id="stage">
  <div id="header"><img src="app-icon.png"><div class="tt">구리 목양교회 앱 사용 안내</div></div>
  <div id="content">
...
<script>
  ...
  const imgs=[document.getElementById('imgA'),document.getElementById('imgB')], bar=document.getElementById('bar');
  let layer=0;
  ...
  function show(i){
    ...
    if(s.type==='title'){
      content.style.display='none'; titleEl.style.display='flex';
      ...
      chip.textContent='STEP '+s.n; capH.textContent=s.t;
...
fs.writeFileSync(path.join(SHOTS, 'player_vertical.html'), html);
const TOTAL_MS = scenes.reduce((a, s) => a + s.ms, 0) + 1200;

(async () => {
  const browser = await chromium.launch({ executablePath: '/opt/pw-browsers/chromium', args: ['--no-sandbox'] });
  const ctx = await browser.newContext({ viewport: { width: 1080, height: 1920 }, recordVideo: { dir: OUTDIR, size: { width: 1080, height: 1920 } } });
  const page = await ctx.newPage();
  await page.goto('file://' + path.join(SHOTS, 'player_vertical.html'), { waitUntil: 'networkidle' });
  await page.waitForTimeout(TOTAL_MS);
  const video = page.video();
  await ctx.close();
  const p = await video.path();
  await browser.close();
  fs.writeFileSync(path.join(OUTDIR, 'webm_vpath.txt'), p);
  console.log('WEBM:', p);
})();
